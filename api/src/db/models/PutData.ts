...
import { sendRequest } from '../../data/utils/commonUtils';
...
export class PutData<IListArgs extends IPutDataArgs>{
  ...
  private async generateStock(detail, vat, citytax) {
    ...
    const totalAmount = detail.count * (detail.unitPrice || 0);
    ...
    return {
      ...
      unitPrice: formatNumber(detail.unitPrice),
  ...
  private async generateStocks() {
    ...
    for (const detail of this.params.details) {
      ...
      const cityTax = detail.amount / (100 + taxPercent) * this.cityTaxPercent;
  ...
  private async generateTransactionInfo() {
    const { stocks, sumAmount, vatAmount, citytaxAmount } = await this.generateStocks()
  ...
  public async run() {
    ...
    if (
      ...
      if (prePutResponse) {
        ...
        response.getInformation = await sendRequest({ url: `${url}/getInformation?lib=${rd}`, method: 'GET', });
      }
    }

    await PutResponses.updatePutResponse(resObj._id, { ...response, customerName: this.params.customerName });
