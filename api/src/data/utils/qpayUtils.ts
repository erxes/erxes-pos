...
export const requestInvoiceDeletion = async (invoiceId: string, accessToken: string, config: IQPayConfig) => {
  const response = await sendRequest({
    method: 'DELETE',
    url: `${config.url}/v2/invoice/${invoiceId}`,
    headers: {
...
export const fetchInvoicePayment = async (invoiceId: string, accessToken: string, config: IQPayConfig) => {
  const response = await sendRequest({
    method: 'POST',
